...
2. C'est le type `sc_signal` qui permet de créer des signaux qui permettent les affectations différées. 
   ...
   Pour mettre en oeuvre ces affectations il y a 3 fonctions. Les fonctions `read`, `write` et `update`. 

   - La fonction `read `permet de voir la valeur actuelle du signal. 
   - La fonction `write` permet de modifier la valeur future du signal (celle qui sera changée à la fin du cycle).
   - La fonction `update` permet de mettre à jour la valeur actuelle qui prend la valeur de la valeur future.

3. Cela est utile par exemple pour les signaux de contrôle, pour savoir quand est-ce qu'un module est prêt à échanger par exemple et cela permet de se synchroniser sur le signal d'horloge.
